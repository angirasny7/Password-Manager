...
public class SplashScreen {
    JFrame frame;
    JLabel image = new JLabel(new ImageIcon("hack.jpg")); // Top image
    JLabel text = new JLabel("PASSWORD MANAGER");
    JLabel text2 = new JLabel("Developer: ANGIRA ");
    JProgressBar progressBar = new JProgressBar();
    JLabel message = new JLabel();

    public SplashScreen() {
        createGUI();
        addImage(); // Add the top image (abc.jpeg)
        addText();
        addProgressBar();
        addMessage();
        addTitle();
    ...
    public void addImage() {
        image.setBounds(100, 20, 400, 180); // Position image at the top
        frame.add(image);
    }
